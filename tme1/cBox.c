...
size_t  boxAllocateds (void) //ajout de l'argument void par bonne pratique
...
Box* boxCreateEmpty (void)
{
  ...
  Box* box = (Box*)malloc( sizeof(Box)); //la boite n'était pas allouée dans la version originale
  
  //utilisation de calloc pour initialiser la mémoire allouée 
  box->name_ = (char*)calloc( 1024, sizeof(char));
  strncpy( box->name_, "Unknown" , 1023); //utilisation de strncpy pour éviter les débordements de mémoire
  box->x1_ =  1;
  box->y1_ =  1;
  box->x2_ = -1;
  box->y2_ = -1;
...
{
  fprintf( stderr, "boxCreate() \"%s\" [%li %li %li %li]\n", name, x1, y1, x2, y2 ); // les variables étaient mal nommées

  Box* box = (Box*)malloc( sizeof(Box));

  //utilisation de calloc pour initialiser la mémoire allouée 
  box->name_ = (char*)calloc(1024, sizeof(char));
  strncpy( box->name_, name, 1023 ); //utilisation de strncpy pour éviter les débordements de mémoire

  //les variables de box étaient mal nommées
  box->x1_ = x1;
  box->y1_ = y1;
  box->x2_ = x2;
  box->y2_= y2;
...
long  boxIsEmpty ( const Box* box ) //la fonction était mal nommée (changement de 'i' en 'I')
...
{  return (boxIsEmpty(box)) ? 0 :  (box->y2_ - box->y1_); } //il manquait la condition pour le cas où la boite est vide
...
long  boxIntersect ( const Box* box1, const Box* box2 )
{
  ...
  //l'intersection n'était pas vérifiée correctement, il fallait utiliser des inégalités strictes
  if (  ((box1->x2_ > box2->x1_)
     && (box1->x1_ < box2->x2_)
     && (box1->y2_ > box2->y1_)
     && (box1->y1_ < box2->y2_) ))

      return 1; //les deux derniers returns étaient inversés
  return 0;
}
...
{
  //il fallait utiliser des - pour les décalages de x1 et y1 au lieu de +
  box->x1_ -= dx1; 
  box->y1_ -= dy1;
  box->x2_ += dx2;
  box->y2_ += dy2;
