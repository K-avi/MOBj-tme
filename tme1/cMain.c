//tme 1 de Mobj de Laila Lahkim Bennani et Ivan Mulot-Radojcic 
#include "cBox.h"


void testFunction ( Box* ab ) 
// changement du type de retour de Box* à void car la fonction retournait directement ab sans l'avoir modifié
{
  ...
  if (boxIntersect(area,ab)) {
    ...
    boxDelete(area); // ajout de la suppression de la boite area pour éviter les fuites de mémoire
    
  } else {
    fprintf( stderr, "  |   No intersection.\n" );

    boxDelete(area); // ajout de la suppression de la boite area pour éviter les fuites de mémoire
    //supression du return ab; car la fonction ne retourne plus rien
  }
}
